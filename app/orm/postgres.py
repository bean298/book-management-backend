...
3. Implement PostgresDBContext - concrete PostgreSQL connection manager with
connection pooling
4. Define AppBaseMixin - reusable mixin providing common fields (created_at,
updated_at, object_status)
"""
...
class AppBaseMixin:
    created_at: Mapped[datetime] = mapped_column(
        ...
        default=lambda: datetime.now().astimezone(UTC),
        nullable=False,
    )

    updated_at: Mapped[datetime | None] = mapped_column(
        DateTime(timezone=True),
        onupdate=lambda: datetime.now().astimezone(UTC),
    )
